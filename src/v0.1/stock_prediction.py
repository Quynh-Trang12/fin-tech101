...
if data.empty:
    ...
    for path in possible_paths:
        if os.path.exists(path):
            print(f"yfinance download returned empty data. Loading cached data from: {path}")
            data = pd.read_csv(path, index_col=0, parse_dates=True)
            data.columns = [col.title() if col.lower() != 'adjclose' else 'Adj Close' for col in data.columns]
...
if test_data.empty:
    ...
    for path in possible_paths:
        if os.path.exists(path):
            print(f"yfinance download returned empty data. Loading cached test data from: {path}")
            test_data = pd.read_csv(path, index_col=0, parse_dates=True)
            test_data.columns = [col.title() if col.lower() != 'adjclose' else 'Adj Close' for col in test_data.columns]
