...
flowchart LR
    ...
    G[Model Comparison]
    H[(GDELT News / FinBERT)]
    I[News Feature Pipeline<br/>C.7]
    J[Statistical + Residual Training<br/>C.6]
...
- **C.6 and C.7** (dashed) do not follow the same lifecycle stages; they are described separately under Pipeline Variations.
...
Because every variation above follows the same execution pipeline, experimental differences can be attributed to the selected configuration rather than differences in the workflow itself.

### Pipelines Outside the Standard Lifecycle

Two experiments reuse individual pipeline components but do not follow the standard lifecycle, because their forecasting or modelling approach does not fit a single train-then-evaluate stage.

`run_c6.py` reuses data preparation for its univariate input windows, but replaces model training and evaluation with an inline sequence: fit an ARIMA baseline, train an LSTM/GRU learner on its residuals, then combine both predictions before reporting metrics. The `c7_*` scripts replace data preparation entirely with a news-specific pipeline that downloads GDELT records, extracts V2Tone and FinBERT sentiment features, and merges them with stock prices into a labelled dataset; model training and evaluation then fit Logistic Regression classifiers for next-day price direction rather than a price forecast. Because both pipelines diverge from the standard lifecycle by design, their results are not directly comparable to Model Comparison outputs from C.1–C.5.
