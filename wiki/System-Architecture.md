...
---
flowchart TB
  DS[("Data Source<br>Yahoo Finance / Local Cache")]
  GD[("Data Source<br>GDELT GKG / FinBERT")]
  CFG["Shared Configuration<br><sub>config.py</sub>"]

  subgraph CORE["Core Forecasting Workflow"]
    DP["Data Processing<br><sub>data_processing.py</sub>"]
    MF["Model Construction<br><sub>model_factory.py</sub>"]
    TR["Model Training<br><sub>train.py</sub>"]
    EV["Model Evaluation<br><sub>test.py</sub>"]
    AUTO["Experiment Automation<br><sub>base_sweep.py</sub><br><sub>run_c4_sweeps.py</sub><br><sub>run_c5_sweeps.py</sub>"]
  ...
  subgraph HYBRID["C.6 Statistical-DL Hybrid<br><sub>run_c6.py</sub>"]
    ARIMA["ARIMA Baseline<br><sub>statsmodels</sub>"]
    RESID["Residual LSTM / GRU Learner"]
    HOUT[("Hybrid Results<br>results/c6")]

    ARIMA --> RESID
    RESID --> HOUT
  end

  subgraph NEWS["C.7 News Sentiment Classification"]
    NPREP["News Data & Feature Pipeline<br><sub>c7_news_*.py, c7_finbert_*.py</sub>"]
    NDS["Classification Dataset<br><sub>c7_dataset.py, c7_preprocessing.py</sub>"]
    NBASE["Logistic Regression Baselines<br><sub>c7_baseline.py</sub>"]
    NOUT[("Classification Results<br>results/c7 / csv-results/c7")]
  ...
  DS --> DP
  DS -.->|"univariate window"| ARIMA
  GD -.-> NPREP
  MF -.->|"reused builder"| RESID
  CFG -.-> CORE
  CFG -.-> HYBRID
  CFG -.-> NEWS
```
...
C.6 and C.7 are separate, self-contained scripts rather than extensions of the Core Forecasting Workflow. C.6 (`run_c6.py`) reuses `data_processing.py` for its univariate input windows and `model_factory.py` for its LSTM/GRU builder, but trains, evaluates, and reports inline in one script instead of going through `train.py`/`test.py`. C.7 does not use `data_processing.py` at all: it builds its own dataset from GDELT news and FinBERT sentiment through a dedicated `c7_*` preprocessing chain, and its baseline model is a Logistic Regression classifier predicting next-day price direction, not a recurrent price forecaster.
...
- Stores C.7 news source settings, including the GDELT query and FinBERT model parameters.

### Statistical-DL Hybrid Forecasting

`run_c6.py` tests whether a residual deep learning model can improve on a statistical ARIMA baseline for univariate price forecasting.

- Runs stationarity tests on the training series to justify the ARIMA differencing order.
- Fits candidate ARIMA models and generates their price predictions.
- Trains LSTM and GRU learners to predict the residual (actual minus ARIMA prediction) using `model_factory.py`.
- Combines the ARIMA prediction with the learned residual to form the final hybrid forecast, and reports metrics against standalone ARIMA and deep learning baselines.

### News Sentiment Classification

The `c7_*` scripts build and evaluate a next-day price direction classifier from financial news sentiment, independent of the price-forecasting workflow.

- Downloads and caches GDELT news records and article titles for the target company.
- Extracts daily sentiment features from GDELT V2Tone and from FinBERT headline-level inference, aligning both to trading days.
- Merges sentiment features with daily stock prices into a labelled classification dataset, then partitions it chronologically.
- Trains and compares Logistic Regression classifiers across market-only, V2Tone, and FinBERT feature sets.
...
| 6    | **Let C.6/C.7 Reuse Components Without Reusing the Workflow** | C.6 forecasts through an ARIMA + residual-learner design and C.7 performs classification rather than forecasting. Neither fits the train-then-evaluate shape the core workflow assumes. | `data_processing.py` and `model_factory.py` stay simple and forecasting-focused, while C.6/C.7 remain free to define their own execution logic without adding conditional branches to the shared workflow. |
